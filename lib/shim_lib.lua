...
function wrap(str, wrap_str, wrap_str_right)
   ...
   if wrap_str_right then
      return (wrap_str..  str  ..wrap_str)
   elseif wrap_str_right == nil then
...
function log(...)
   ...
   if log_quiet ~= true then
      _protected.print( wrap( gmodule_name, "[" ), message_string)
   end
end
...
-- The function name is very short on purpose.
-- Debug/programmer oriented functions are important,
-- so it should be very quick to type out and use
function dlog(...)
...
function section(name)
   safe_assert(name, "A name for the section must be provided")
   log("["..name.." Begin]")
end

function end_seciton(name)
   name = name or  "of Section"
   log("[End "..name.."]")
end
...
function file_exists(file)
   safe_assert( type(file) == "string", "file must be of type 'string'")
...
function table.get_keys(target)
   local keystring = ""
